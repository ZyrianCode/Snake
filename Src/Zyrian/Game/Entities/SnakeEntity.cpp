...
{
	for (int i = 2; i < gameStats->Score; i++)
	{
...
void Snake::Snake::StartGrowth()
{
	...
	SnakeEntity[gameStats->Score]->Location = Point(SnakeEntity[gameStats->Score - 1]->Location.X + (Step *direction->X), SnakeEntity[gameStats->Score - 1]->Location.Y - (Step * direction->Y));
